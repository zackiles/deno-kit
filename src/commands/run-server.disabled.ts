  ...
  try {
    ...
    return await withContext(requestContext, async () => {
      try {
        ...
        // Record error in the span
        const errorObj = serverError instanceof Error ? serverError : new Error(String(serverError))
...
function handleWebSocket(request: Request): Response {
  ...
  socket.onerror = (e) => {
    withContext(wsConnectionContext, () => {
      const errorMessage = e instanceof ErrorEvent ? e.message : 'Unknown WebSocket error'
...
function handler(request: Request): Response | Promise<Response> {
  ...
  const rootSpan = createSpan('http.server.request', {
    ...
    attributes: {
      ...
      ...(traceContext.traceparent ? { 'trace.parent': traceContext.traceparent } : {}),
      ...(traceContext.tracestate ? { 'trace.state': traceContext.tracestate } : {}),
    },
  })
