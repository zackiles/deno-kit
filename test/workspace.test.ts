...
Deno.test('Workspace functionality', async (t) => {
  ...
  const templatesDir = await Deno.makeTempDir({
    prefix: 'deno-kit-test-templates-',
  })
  ...
  await t.step(
    'compileAndWriteTemplates - replaces placeholders correctly',
    async () => {
      ...
      assertStringIncludes(
        readmeContent,
        'MIT',
        'PACKAGE_LICENSE should be replaced',
      )
      assertStringIncludes(
        readmeContent,
  ...
  await t.step(
    'toJSON - returns valid JSON with all required properties',
    async () => {
      ...
      assertEquals(
        parsedJson.id,
        workspace.id,
        'JSON should contain correct ID',
      )
      assertEquals(
        parsedJson.name,
        workspace.name,
  ...
  await t.step(
    'compileAndWriteTemplates - overrides template values correctly',
    async () => {
      const readmePath = join(workspace.path, 'README.md')
      try {
        ...
        assertStringIncludes(
          readmeContent,
          '# Override Project',
          'PROJECT_NAME should be overridden',
        )
        ...
        console.log(
          'Error testing template overrides:',
          (error as Error).message,
        )
        assertEquals(
          false,
          true,
          'Should not throw error when overriding template values',
        )
      }
    },
  )
  ...
  await t.step(
    'compileAndWriteTemplates - processes nested directory templates',
    async () => {
  ...
  await t.step(
    'writeFile - writes content to workspace correctly',
    async () => {
  ...
  await t.step('workspace ID - has valid format', () => {
    ...
    assertEquals(
      id.length,
      64,
      'Workspace ID should be 64 characters (SHA-256 hash)',
    )
    // Should only contain valid hex characters
    assertEquals(
      /^[0-9a-f]+$/.test(id),
      true,
      'Workspace ID should only contain hex characters',
    )
  })

  // Step 8: Test handling of missing placeholders in templates
  await t.step(
    'compileAndWriteTemplates - preserves unmatched placeholders',
    async () => {
  ...
  await t.step(
    'getGitUserInfo - retrieves git user information safely',
    async () => {
      ...
      assertEquals(
        typeof userName,
        'string',
        'Git user name should be a string',
      )
      assertEquals(
        typeof userEmail,
        'string',
        'Git user email should be a string',
      )
    },
  )
  ...
  await t.step(
    'templatesPath validation - rejects invalid templates paths',
    async () => {
  ...
  await t.step(
    'writeFile - prevents writing outside workspace directory',
    async () => {
  ...
  await t.step('writeFile - creates nested directories as needed', async () => {
    ...
    assertEquals(
      fileContent,
      content,
      'Content should be written to deeply nested path',
    )

    // Check that intermediate directories were created
    const stat = await Deno.stat(
      join(workspace.path, 'deep/nested/path/structure'),
    )
    assertEquals(
      stat.isDirectory,
      true,
      'Intermediate directories should be created',
    )
  })
  ...
  await t.step('writeFile - respects create=false flag', async () => {
    ...
    try {
      ...
      assertEquals(
        true,
        false,
        'Non-existent file should not be created with create=false',
      )
    } catch (error) {
    ...
    const updatedContent = await Deno.readTextFile(
      join(workspace.path, testPath),
    )
    assertEquals(
      updatedContent,
      newContent,
      'File should be overwritten with create=true',
    )
  })

  // Step 14: Test handling of empty template values
  await t.step(
    'compileAndWriteTemplates - rejects empty template values',
    async () => {
  ...
  await t.step(
    'backup functionality - backup path is accessible and valid',
    async () => {
      ...
      try {
        ...
        assertEquals(
          stat.isDirectory,
          true,
          'Backup path should be a directory',
        )
  ...
  await t.step(
    'compileAndWriteTemplates - handles complex placeholder patterns',
    async () => {
  ...
  await t.step(
    'createWorkspace - creates and persists workspace configuration',
    async () => {
      ...
      try {
        ...
        assertEquals(
          config.id,
          workspace.id,
          'Config file should contain correct workspace ID',
        )
        assertEquals(
          config.name,
          workspace.name,
          'Config file should contain correct workspace name',
        )
        assertExists(
          config.workspaceFiles,
          'Config file should list workspace files',
        )
        assertExists(
          config.templateFiles,
          'Config file should list template files',
        )
        ...
        assertEquals(
          true,
          false,
          `Failed to verify workspace config file: ${error}`,
        )
      }
    },
  )
  ...
  await t.step(
    'createWorkspace - validates workspace path requirements',
    async () => {
      ...
      try {
        ...
        assertEquals(
          true,
          false,
          'Should not be able to create workspace in banned directory',
        )
      } catch (error) {
  ...
  await t.step(
    'createWorkspace - handles empty template directory',
    async () => {
      // Create an empty templates directory
      const emptyTemplatesDir = await Deno.makeTempDir({
        prefix: 'deno-kit-empty-templates-',
      })
  ...
  await t.step(
    'createWorkspace - validates workspace directories',
    async () => {
  ...
  await t.step(
    'backup - creates backup copies of workspace files',
    async () => {
      ...
      try {
        ...
        assertEquals(
          fileInfo.isFile,
          true,
          `Test file should exist: ${testFilePath}`,
        )
        ...
        assertEquals(
          true,
          false,
          `Failed to stat test file: ${testFilePath}, error: ${err}`,
        )
      }
        ...
        if (backupPath.includes(testFileName)) {
          ...
          try {
            ...
            assertEquals(
              fileInfo.isFile,
              true,
              `Backup file should exist on disk: ${backupPath}`,
            )
            ...
            assertEquals(
              diskContent,
              testContent,
              'Backup content on disk should match original',
            )
          } catch (err) {
            assertEquals(
              true,
              false,
              `Failed to stat backup file: ${backupPath}, error: ${err}`,
            )
          }
          break
        }
      ...
      assertEquals(
        foundTestFile,
        true,
        `Should find test file ${testFileName} in backups`,
      )
    },
  )
  ...
  await t.step('backup - preserves subdirectory structure', async () => {
    ...
    try {
      ...
      assertEquals(
        fileInfo.isFile,
        true,
        `Nested file should exist: ${fullPath}`,
      )
    } catch (err) {
      assertEquals(
        true,
        false,
        `Failed to stat nested file: ${fullPath}, error: ${err}`,
      )
    }
    ...
    for (const [backupPath, content] of backupFiles.entries()) {
      if (
        backupPath.includes(nestedFileName) && backupPath.includes('nested')
      ) {
        ...
        assertEquals(
          content,
          nestedContent,
          'Nested backup content should match original',
        )
        break
      }
    }
    ...
    assertEquals(
      foundNestedFile,
      true,
      `Should find nested file ${nestedFileName} in backups`,
    )
    ...
    if (foundNestedFile && nestedBackupPath) {
      ...
      const backupDir = nestedBackupPath.substring(
        0,
        nestedBackupPath.lastIndexOf('/'),
      )
      ...
      try {
        ...
        assertEquals(
          true,
          false,
          `Failed to stat backup directory: ${backupDir}, error: ${err}`,
        )
      }
    }
  })
  ...
  await t.step('backup - excludes template files', async () => {
    ...
    const nonTemplateContent =
      'This is not a template file and should be backed up'
    ...
    try {
      const nonTemplateInfo = await Deno.stat(
        join(workspace.path, nonTemplateFile),
      )
      assertEquals(
        nonTemplateInfo.isFile,
        true,
        'Non-template file should exist in workspace',
      )

      const templateInfo = await Deno.stat(
        join(workspace.path, templateFileName),
      )
      assertEquals(
        templateInfo.isFile,
        true,
        'Modified template file should exist in workspace',
      )
    ...
    // Add debugging - dump the backup files contents
    console.log(
      'Debug - Backup files count in template test:',
      backupFiles.size,
    )
    console.log('Debug - Backup paths:')
    ...
    assertEquals(
      foundNonTemplateFile,
      true,
      'Should find non-template file in backups',
    )
    ...
    assertEquals(
      foundModifiedTemplate,
      true,
      'Should find modified file in backups',
    )
    ...
    for (const [backupPath] of backupFiles.entries()) {
      ...
      if (filename === 'README.md') {
        foundReadme = true
        console.log(
          'Debug - Found README.md in backup when it should be excluded:',
          backupPath,
        )
        break
      }
    ...
    assertEquals(
      foundReadme,
      false,
      'README.md should be excluded from backups by filename',
    )
    ...
    assertEquals(
      foundTemplateDirFiles,
      false,
      'No template directory files should be in backup',
    )
  })

  // Step 24: Test loadWorkspace - loads an existing workspace from configuration
  await t.step(
    'loadWorkspace - loads workspace from configuration file',
    async () => {
      ...
      assertEquals(
        loadedWorkspace.id,
        workspace.id,
        'Loaded workspace ID should match original',
      )
      assertEquals(
      ...
      const readContent = await Deno.readTextFile(
        join(loadedWorkspace.path, testFilePath),
      )
      assertEquals(
        readContent,
        testContent,
        'Should be able to write files to loaded workspace',
      )
    },
  )
  ...
  await t.step(
    'JSON persistence - ensures all stored paths are relative',
    async () => {
  ...
  try {
    ...
    if (
      workspace.backupsPath && !(await isBannedDirectory(workspace.backupsPath))
    ) {
...
Deno.test('Workspace with JSONC config file', async (t) => {
  ...
  await t.step('create with .jsonc config file', () => {
    ...
    assertExists(
      ...
      'Config file with .json extension should exist',
    )
  })
